...
YouTube queue player with autoplay, push notifications, and a rich web UI — all
...
1. **PiPulse API** (when enabled) — live fetch from `GET /api/schedule/blocks`
2. **Local SQLite** (always) — user-managed via web UI block editor
3. **Raw block name** (fallback) — current behavior if neither configured

### PiPulse Integration Settings

Settings page toggle:
- **PiPulse Integration:** ON/OFF
- **PiPulse Host:** `10.0.0.110` (default)
...
- Graceful fallback — never crashes, never shows error banners

### New PiPulse API Endpoint

**`GET /api/schedule/blocks`** — Returns merged block metadata from schedule.yaml + blocks.yaml.
...
Data merged from:
- `schedule.yaml` → display_name, block_start, fire_hour/minute, keystone, goal_tag
- `blocks.yaml` → emoji, verb, tagline, block_type, energy, purpose
...
- "Import from PiPulse" button: one-click pull from API → populates local table
...
1. Pushover Notifications (optional)
   ...
   User Key:  ▌ (from https://pushover.net/dashboard)

   [Testing...] ✓ Notification sent! Check your phone.
...
- Print status summary:
  ✓ PiCast v1.0.0 running on port 5050
  ✓ mpv connected
  ⚠ Pushover: not configured (run picast-setup later)
  ⚠ YouTube auth: not configured (recommended)
  ✓ Web UI: http://picast.local:5050
```

### Key Principles

- All optional steps are **skippable** — the app works without them
- `picast-setup` is a standalone command that can be run anytime
- Post-install health check gives immediate confidence
- Display rotation is NOT in install — it's in Settings web UI
...
```
┌──────────────────────────────────────┐
│ 🔔 Pushover Notifications      OFF  │
│                                      │
│ ▸ Setup Guide                        │
│   1. Create app: pushover.net/apps   │
│   2. Copy API Token + User Key       │
│   3. SSH: picast-setup               │
│   Or edit ~/.config/picast/picast.toml│
└──────────────────────────────────────┘
```

Cards with hints:
- **Pushover** — token setup steps + link to pushover.net
- **YouTube Auth** — chromium cookie steps
- **PiPulse Integration** — host/port config + test button
- **Autoplay Blocks** — block editor + "Import from PiPulse" button
...
- End-to-end test on Pi: fresh config → setup wizard → all features working
...
- Schema v10 migration must be tested before pool page changes (S1 → S2)
- picast-setup must exist before install-pi.sh can call it (S4 → S3, or stub it)
...
...can run `install-pi.sh`, optionally configure everything through the wizard or web UI, and have a fully functional PiCast with rich autoplay blocks, push notifications, and YouTube auth — without editing any config files.
