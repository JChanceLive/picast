"""Tests for AutopilotEngine — AI-enhanced video selection and queue management (v2)."""
...
from picast.server.autopilot_engine import AutopilotEngine, _block_to_mood, _weighted_shuffle
...
def _save_profile(db, profile_dict, generated_at="2026-03-10T06:00:00"):
    ...
    db.execute(
        ...
        (profile_json, generated_at, profile_dict.get("version", 2)),
    )
    db.commit()
...
class TestEngineState:
    def test_starts_stopped(self, engine):
        ...
        assert engine.current_mood is None
...
class TestStatus:
    def test_status_default(self, engine):
        ...
        assert status["current_mood"] is None
    ...
    def test_status_after_start(self, engine, pool):
        ...
        assert status["current_mood"] == "chill"
        assert status["queue_depth"] > 0
...
class TestSelection:
    def test_select_from_empty_pool(self, engine):
        engine.start()
        result = engine.select_next("chill")
    ...
    def test_select_without_mood(self, engine):
        engine.start()
    ...
    def test_select_scores_across_all_blocks(self, engine, pool):
        """Freeform mode: videos from different blocks should all be scored."""
        _seed_pool(pool, "morning", count=3, prefix="mor")
        _seed_pool(pool, "evening", count=3, prefix="eve")
        engine.start()

        result = engine.select_next("chill")
        assert result is not None
        queue = engine.get_queue_preview()
        # Queue should contain videos from both blocks
        block_names = {v.get("block_name", "") for v in queue}
        block_names.add(result.get("block_name", ""))
        assert len(block_names) > 0  # At least some videos found
...
class TestScoring:
    ...
    def test_liked_videos_score_higher(self, engine, pool, db):
        ...
        queue = engine._score_library("chill")
    ...
    def test_duration_penalty(self, engine, pool, profile, db):
        """Videos exceeding mood max_duration should get penalized."""
        _save_profile(
            db,
            _make_profile(
                energy_profiles={
                    "chill": {"max_duration": 1000, "genres": ["ambient"], "tempo": "slow", "description": "test"},
                    "focus": {"max_duration": 5400, "genres": [], "tempo": "steady", "description": "test"},
                    "vibes": {"max_duration": 3600, "genres": [], "tempo": "any", "description": "test"},
                },
            ),
        )
        profile.load(db)
    ...
    def test_avoid_patterns_filter(self, engine, pool, profile, db):
        """Videos matching avoid patterns should be excluded."""
        _save_profile(db, _make_profile(avoid_patterns=["asmr", "mukbang"]))
        profile.load(db)

        pool.add_video(
            "morning",
            "https://www.youtube.com/watch?v=good00aaaaa",
            title="Relaxing Ambient",
            tags="ambient",
        )
        pool.add_video(
            "morning",
            "https://www.youtube.com/watch?v=bad000aaaaa",
            title="ASMR Whisper",
            tags="asmr",
        )
        engine.start()
        ...
        assert "good00aaaaa" in video_ids
        assert "bad000aaaaa" not in video_ids

    def test_creator_affinity_boosts_score(self, engine, pool, profile, db):
        """Videos from preferred creators should score higher."""
        _save_profile(db, _make_profile(
            creator_affinity={"Chillhop": 1.8},
        ))
        profile.load(db)

        pool.add_video(
            "morning",
            "https://www.youtube.com/watch?v=chill0aaaaa",
            title="Chillhop Music Session",
            tags="ambient",
        )
        pool.add_video(
            "morning",
            "https://www.youtube.com/watch?v=other0aaaaa",
            title="Unknown Artist Session",
            tags="ambient",
        )
        engine.start()
        ...
        queue = engine._score_library("chill")
        scores_by_id = {v["video_id"]: v["score"] for v in queue}
        assert scores_by_id["chill0aaaaa"] > scores_by_id["other0aaaaa"]
...
class TestQueueManagement:
    def test_mood_change_clears_queue(self, engine, pool):
        _seed_pool(pool, "morning", count=5)
        _seed_pool(pool, "evening", count=5, prefix="eve")
        engine.start()
        engine.on_mood_change("chill")
        chill_queue = engine.get_queue_preview()
        assert len(chill_queue) > 0

        engine.on_mood_change("focus")
        assert engine.current_mood == "focus"

    def test_block_change_maps_to_mood(self, engine, pool):
        """on_block_change should map block names to moods."""
        _seed_pool(pool, "morning", count=5)
        engine.start()
        engine.on_block_change("morning-foundation")
        assert engine.current_mood == "chill"

        engine.on_block_change("creation-stack")
        assert engine.current_mood == "focus"
...
class TestBlockToMood:
    def test_morning_maps_to_chill(self):
        assert _block_to_mood("morning-foundation") == "chill"

    def test_evening_maps_to_chill(self):
        assert _block_to_mood("evening-transition") == "chill"

    def test_creation_maps_to_focus(self):
        assert _block_to_mood("creation-stack") == "focus"

    def test_midday_maps_to_vibes(self):
        assert _block_to_mood("midday-reset") == "vibes"

    def test_unknown_defaults_to_vibes(self):
        assert _block_to_mood("unknown-block") == "vibes"
...
class TestLogging:
    ...
    def test_mood_change_logged(self, engine, pool, db):
        _seed_pool(pool, "morning")
        engine.start()
        engine.on_mood_change("chill")
        rows = db.fetchall("SELECT * FROM autopilot_log WHERE action = 'mood_change'")
        assert len(rows) == 1
        assert rows[0]["block_name"] == "chill"
...
class TestAPI:
    ...
    def test_autopilot_set_mood(self, client, app_with_autopilot):
        """Test the new mood endpoint."""
        pool = AutoPlayPool(app_with_autopilot.db)
        _seed_pool(pool, "morning", count=3)

        client.post("/api/autopilot/toggle")
        resp = client.post("/api/autopilot/mood", json={"mood": "chill"})
        assert resp.status_code == 200
        data = resp.get_json()
        assert data["ok"] is True
        assert data["mood"] == "chill"

    def test_autopilot_set_mood_invalid(self, client):
        resp = client.post("/api/autopilot/mood", json={"mood": "invalid"})
    ...
    def test_profile_upload_missing_profile(self, client):
        resp = client.post(
            "/api/autopilot/profile",
            json={"generated_at": "2026-03-10T06:00:00"},
        )
        assert resp.status_code == 400
    ...
    def test_feedback_more(self, client, app_with_autopilot):
        ...
        resp = client.post(
            ...
            json={"signal": "more", "video_id": "vid000aaaaa", "mood": "chill"},
        )
        assert resp.status_code == 200
        data = resp.get_json()
        assert data["ok"] is True
    ...
    def test_feedback_less(self, client):
        resp = client.post(
            "/api/autopilot/feedback",
            json={"signal": "less", "video_id": "vid000aaaaa", "mood": "chill"},
        )
        assert resp.status_code == 200
...
class TestEngineNewMethods:
    ...
    def test_record_feedback(self, engine, db):
        engine.record_feedback("vid000aaaaa", "more", "chill")
        row = db.fetchone(
...
def _make_profile_with_discovery():
    """Build a v2 profile with discovery queries."""
    return {
        "version": 2,
        "generated_at": "2026-03-10T06:00:00",
        "global_preferences": {"genre_weights": {"ambient": 0.9}},
        "energy_profiles": {
            "chill": {"genres": ["ambient"], "max_duration": 1800, "tempo": "slow", "description": "test"},
            "focus": {"genres": [], "max_duration": 5400, "tempo": "steady", "description": "test"},
            "vibes": {"genres": [], "max_duration": 3600, "tempo": "any", "description": "test"},
        },
        "discovery_queries": ["chill ambient music"],
    }
...
class TestDiscoveryIntegration:
    ...
    def test_discovery_fills_slots_when_enabled(self, discovery_engine, db, pool, mock_discovery):
        ...
        result = discovery_engine.select_next("chill")
        assert result is not None
...
class TestEngineFleet:
    ...
    def test_select_next_fleet_logs_push(self, mock_urlopen, fleet_engine, pool, db):
        ...
        fleet_engine.start()
        fleet_engine.on_mood_change("chill")
