...
#   PICAST_VERSION=0.28.0   Install a specific version
...
#   PICAST_SKIP_SETUP=1      Skip optional setup prompt (Phase 2)
...
# Generated by install-pi.sh on $(date -u '+%Y-%m-%d %H:%M UTC')
#
# All optional features are disabled by default.
# Enable them here, via the web UI settings page, or by running: picast-setup
...
# Option 1: Browser cookies (sign into Chromium on this Pi, then uncomment)
...
echo -e "\n${GREEN}Phase 1 complete!${NC} Core system installed."
...
if [ "$PICAST_SKIP_SETUP" = "1" ]; then
    echo -e "  ${DIM}Skipping optional setup (PICAST_SKIP_SETUP=1)${NC}"
    echo "  Run ${BOLD}picast-setup${NC} later to configure Pushover, YouTube auth, and PiPulse."
elif [ -t 0 ]; then
    # Only prompt if running interactively (stdin is a terminal)
    echo ""
    echo "  PiCast works out of the box, but optional features make it better:"
    echo ""
    echo "    ${BOLD}Pushover${NC}       - Push notifications to your phone"
    echo "    ${BOLD}YouTube Auth${NC}   - Play age-restricted and region-locked videos"
    echo "    ${BOLD}PiPulse${NC}        - Rich autoplay blocks with schedule metadata"
    echo ""

    read -rp "  Configure optional features now? (y/N) " SETUP_ANSWER
    if [[ "$SETUP_ANSWER" =~ ^[Yy]$ ]]; then
        if command -v picast-setup &> /dev/null; then
            echo ""
            picast-setup
        else
            echo ""
            warn "picast-setup not yet available in this version."
            echo "    Configure via the web UI settings page or edit:"
            echo "    ~/.config/picast/picast.toml"
            echo ""
            echo "    ${DIM}picast-setup will be available in a future update.${NC}"
        fi
    else
        echo ""
        echo "  No problem! You can configure these anytime:"
        echo "    - Web UI: http://$(hostname 2>/dev/null || echo 'picast').local:5050/settings"
        echo "    - CLI:    picast-setup"
        echo "    - File:   ~/.config/picast/picast.toml"
    fi
else
    # Non-interactive (piped from curl) - skip setup prompt
    echo -e "  ${DIM}Non-interactive install detected, skipping setup prompt.${NC}"
    echo "  Run ${BOLD}picast-setup${NC} after install to configure optional features."
fi
...
if [ "$PICAST_SKIP_SERVICE" = "1" ]; then
    echo ""
    echo "  Service setup was skipped. Start manually:"
    echo "    picast-server"
    echo ""
else
    # Start (or restart) the service
    echo ""
    echo -e "  Starting PiCast service..."
    sudo systemctl restart picast 2>/dev/null || sudo systemctl start picast 2>/dev/null || true
    sleep 3
    ...
    if command -v curl &> /dev/null; then
        HEALTH_JSON=$(curl -sf "http://localhost:5050/api/health" 2>/dev/null || echo "")
        if [ -n "$HEALTH_JSON" ]; then
            API_STATUS="healthy"
        fi
    fi

    # Read config to check what's enabled
    CONFIG_FILE="$HOME/.config/picast/picast.toml"
    PUSHOVER_ENABLED="not configured"
    YOUTUBE_AUTH="not configured"
    PIPULSE_ENABLED="not configured"
    AUTOPLAY_ENABLED="not configured"

    if [ -f "$CONFIG_FILE" ]; then
        # Check for uncommented enabled lines (simple grep, not a full TOML parser)
        if grep -q '^\[pushover\]' "$CONFIG_FILE" 2>/dev/null && \
           grep -q '^enabled = true' "$CONFIG_FILE" 2>/dev/null; then
            PUSHOVER_ENABLED="enabled"
        fi
        if grep -q '^ytdl_cookies_from_browser' "$CONFIG_FILE" 2>/dev/null || \
           grep -q '^ytdl_po_token' "$CONFIG_FILE" 2>/dev/null; then
            YOUTUBE_AUTH="configured"
        fi
        if grep -q '^\[pipulse\]' "$CONFIG_FILE" 2>/dev/null && \
           grep -q '^enabled = true' "$CONFIG_FILE" 2>/dev/null; then
            PIPULSE_ENABLED="enabled"
        fi
        if grep -q '^\[autoplay\]' "$CONFIG_FILE" 2>/dev/null && \
           grep -q '^enabled = true' "$CONFIG_FILE" 2>/dev/null; then
            AUTOPLAY_ENABLED="enabled"
        fi
    fi

    # Check for Chromium cookies (YouTube auth readiness)
    if [ -d "$HOME/.config/chromium/Default" ]; then
        if [ "$YOUTUBE_AUTH" = "not configured" ]; then
            YOUTUBE_AUTH="cookies available (not enabled in config)"
        fi
    fi
    ...
    echo ""
    echo -e "${BLUE}  ┌─────────────────────────────────────────┐${NC}"
    echo -e "${BLUE}  │         ${BOLD}PiCast Installation Summary${NC}${BLUE}       │${NC}"
    echo -e "${BLUE}  └─────────────────────────────────────────┘${NC}"
    echo ""

    # Service status
    if [ "$SERVICE_STATUS" = "running" ]; then
        echo -e "  ${GREEN}✓${NC} PiCast service         ${GREEN}running${NC}"
    else
        echo -e "  ${RED}✗${NC} PiCast service         ${RED}$SERVICE_STATUS${NC}"
        echo -e "    ${DIM}Check: journalctl -u picast -n 20${NC}"
    fi

    # API health
    if [ "$API_STATUS" = "healthy" ]; then
        echo -e "  ${GREEN}✓${NC} API health             ${GREEN}healthy${NC}"
    else
        echo -e "  ${RED}✗${NC} API health             ${RED}unreachable${NC}"
        echo -e "    ${DIM}Service may still be starting. Wait 10s and try:${NC}"
        echo -e "    ${DIM}curl http://localhost:5050/api/health${NC}"
    fi

    # mpv player
    if [ "$API_STATUS" = "healthy" ] && echo "$HEALTH_JSON" | python3 -c "import sys,json; d=json.load(sys.stdin); sys.exit(0 if d.get('mpv_connected') else 1)" 2>/dev/null; then
        echo -e "  ${GREEN}✓${NC} mpv player             ${GREEN}connected${NC}"
    elif [ "$API_STATUS" = "healthy" ]; then
        echo -e "  ${YELLOW}⚠${NC} mpv player             ${YELLOW}not connected${NC}"
        echo -e "    ${DIM}Normal on headless/SSH. Connects when HDMI display is active.${NC}"
    fi
    ...
    if [ "$PUSHOVER_ENABLED" = "enabled" ]; then
        echo -e "  ${GREEN}✓${NC} Pushover notifications ${GREEN}enabled${NC}"
    else
        echo -e "  ${DIM}○${NC} Pushover notifications ${DIM}$PUSHOVER_ENABLED${NC}"
    fi

    if [ "$YOUTUBE_AUTH" = "configured" ]; then
        echo -e "  ${GREEN}✓${NC} YouTube auth           ${GREEN}configured${NC}"
    elif [ "$YOUTUBE_AUTH" = "cookies available (not enabled in config)" ]; then
        echo -e "  ${YELLOW}⚠${NC} YouTube auth           ${YELLOW}cookies found, enable in config${NC}"
    else
        echo -e "  ${DIM}○${NC} YouTube auth           ${DIM}$YOUTUBE_AUTH${NC}"
    fi

    if [ "$PIPULSE_ENABLED" = "enabled" ]; then
        echo -e "  ${GREEN}✓${NC} PiPulse integration    ${GREEN}enabled${NC}"
    else
        echo -e "  ${DIM}○${NC} PiPulse integration    ${DIM}$PIPULSE_ENABLED${NC}"
    fi

    if [ "$AUTOPLAY_ENABLED" = "enabled" ]; then
        echo -e "  ${GREEN}✓${NC} AutoPlay pools         ${GREEN}enabled${NC}"
    else
        echo -e "  ${DIM}○${NC} AutoPlay pools         ${DIM}$AUTOPLAY_ENABLED${NC}"
    fi
fi
...
echo ""
echo -e "${BLUE}  ┌─────────────────────────────────────────┐${NC}"
echo -e "${BLUE}  │            ${BOLD}Quick Reference${NC}${BLUE}               │${NC}"
echo -e "${BLUE}  └─────────────────────────────────────────┘${NC}"
echo ""
echo -e "  ${BOLD}Web UI:${NC}    http://${HOSTNAME}.local:5050"
echo -e "  ${BOLD}Settings:${NC}  http://${HOSTNAME}.local:5050/settings"
echo -e "  ${BOLD}API:${NC}       http://${HOSTNAME}.local:5050/api/health"
echo ""
...
echo "    picast-setup               # Interactive wizard"
echo "    ~/.config/picast/picast.toml  # Config file"
echo ""
...
echo ""
echo -e "  ${BOLD}On your Mac:${NC}"
echo "    pip install picast"
echo "    picast                     # TUI dashboard"
echo ""
